...
for table in $TABLES; do
	...
	_end=`date '+%s'`
	echo "mysqldump --skip-lock-tables --skip-add-locks --skip-add-drop-table --no-create-info -h$LOCAL_DB_HOST -P$LOCAL_DB_PORT -u$LOCAL_DB_USER -p$LOCAL_DB_PASS $LOCAL_DB_NAME $table \> $DUMP_FILE <<-- elapsed time $(expr $_end - $_start)"
...
for table in $TABLES; do
	...
	_end=`date '+%s'`
	echo "mysql -h$DEST_DB_HOST -P$DEST_DB_PORT -u$DEST_DB_USER -p$DEST_DB_PASS $DEST_DB_NAME \< $DUMP_FILE <<-- elapsed time $(expr $_end - $_start)"

	_start=`date '+%s'`
	mysql -h$DEST_DB_HOST -P$DEST_DB_PORT -u$DEST_DB_USER -p$DEST_DB_PASS $DEST_DB_NAME -e "delete from $table"
	_end=`date '+%s'`
	echo "mysql -h$DEST_DB_HOST -P$DEST_DB_PORT -u$DEST_DB_USER -p$DEST_DB_PASS $DEST_DB_NAME -e "delete from $table" <<-- elapsed time $(expr $_end - $_start)"
	_start=`date '+%s'`
	mysql -h$DEST_DB_HOST -P$DEST_DB_PORT -u$DEST_DB_USER -p$DEST_DB_PASS $DEST_DB_NAME -e "insert into $table select * from ${table}_I"
	_end=`date '+%s'`
	echo "mysql -h$DEST_DB_HOST -P$DEST_DB_PORT -u$DEST_DB_USER -p$DEST_DB_PASS $DEST_DB_NAME -e "insert into $table select \* from ${table}_I" <<-- elapsed time $(expr $_end - $_start)"
	_start=`date '+%s'`
	mysql -h$DEST_DB_HOST -P$DEST_DB_PORT -u$DEST_DB_USER -p$DEST_DB_PASS $DEST_DB_NAME -e "drop table ${table}_I"
	_end=`date '+%s'`
	echo "mysql -h$DEST_DB_HOST -P$DEST_DB_PORT -u$DEST_DB_USER -p$DEST_DB_PASS $DEST_DB_NAME -e "drop table ${table}_I" <<-- elapsed time $(expr $_end - $_start)"
done
